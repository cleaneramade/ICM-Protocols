...
// Single source of truth for the version — read from package.json so the health
// endpoint can never drift from the real release version.
...
router.add('GET', '/api/health', async (req, res) => {
  ...
  sendJson(res, 200, { ok: true, version: VERSION, node: process.version, paths: checks });
});
...
// Read-scope over the preview tunnel (audit LOCAL-1). The ephemeral public URL
// is guarded only by URL-unguessability, so it serves ONLY the governance board
// — which carries no filesystem paths, no secrets, no other-project data. Every
// other read stays local-only: /api/projects & /api/config (home paths),
// /api/env (per-project key names), /api/brain/* (private notes), /api/skills,
// /api/profiles, /api/backups, and especially /api/security/* (which enumerates
// each project's unpatched holes). Writes are already blocked above.
const TUNNEL_GET_ALLOW = new Set(['/api/health', '/api/board', '/api/deleted']);
...
function makeServer(port) {
  return http.createServer(async (req, res) => {
    ...
    // Tunnel read-scope: only the board is public over the preview URL; every
    // other API read is local-only (see TUNNEL_GET_ALLOW above).
    if (req.icmRemote && url.pathname.startsWith('/api/') && !TUNNEL_GET_ALLOW.has(url.pathname)) {
      return sendJson(res, 403, { ok: false, code: 'REMOTE_BLOCKED', error: 'This section is local-only and is not served over the remote preview tunnel. Open it on the machine running ICM Protocols.' });
    }
    ...
    // Belt-and-suspenders: serveStatic guards its own decode, but never let a
    // static path reject unhandled (that would take down the process).
    await serveStatic(req, res, url).catch(() => {
