...
const COLOR_WORD = { red: 'At risk', yellow: 'Needs work', green: 'Protected', gray: 'Not audited' };
const TALLY_WORD = { red: 'at risk', yellow: 'need work', green: 'protected', gray: 'not audited' };
...
function renderList() {
  ...
  for (const p of state.projects) tally[p.effectiveColor] = (tally[p.effectiveColor] || 0) + 1;
  const total = state.projects.length;
  const chips = el('div', { class: 'sec-list-tally' });
  for (const c of ['red', 'yellow', 'green', 'gray']) {
    if (!tally[c]) continue;
