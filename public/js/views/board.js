...
import { loadBoard, boardState, onBoardChange, updateRow, addRow, deleteRow, deleteNewRow, setRuleColor, markDirty } from '../boardstore.js';
  ...
  dialog({
    ...
    actions: [
      ...
      { label: isNew ? 'Add' : 'Apply', kind: 'primary', onClick: () => {
        ...
        // Unsaved rows (added or color-moved) have id:null — an id lookup would
        // hit the FIRST null-id row in the section, editing the wrong rule.
        else if (row.id == null) { row.cells = cells; row.icon = picker.getIcon(); markDirty(); }
