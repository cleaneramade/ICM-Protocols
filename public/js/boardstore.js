...
export function setRuleAccess(sectionKey, id, level) {
  const row = state.sections[sectionKey].rows.find((r) => r === id || r.id === id);
  if (row) { row.access = level; markDirty(); }
}
...
export function setRuleColor(fromKey, row, toKey) {
  ...
  state.sections[toKey].rows.push({ id: null, cells, access: row.access, category: row.category, icon: row.icon });
  markDirty();
}
