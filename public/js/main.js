...
async function mount(key) {
  ...
  // Each view gets its own disposable wrapper (display:contents — invisible to
  // layout). Views keep a reference to it and guard async re-renders with
  // document.contains(container); mounting into #view directly would make that
  // guard always-true and let a stale listener paint the previous view over
  // the current one (e.g. the Rules board clobbering Connections after save).
  const host = clear(document.getElementById('view'));
  const container = el('div', { class: 'view-host' });
