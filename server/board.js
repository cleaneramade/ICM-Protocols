...
// Shared with the frontend (the module is dependency-free): built-in rule ID →
// category. Rules keep their category across color moves, deletes, and restores
// even when the sidecar has no entry for them yet.
import { RULE_ID_TO_CAT } from '../public/js/categories.js';
  ...
  if (toRecord.length) {
    ...
    for (const d of toRecord) {
      const rec = {
        ...
        category: oldRuleCat[d.id] || RULE_ID_TO_CAT[d.id] || null,
...
export async function getBoard() {
  ...
  for (const key of ['green', 'yellow', 'red']) {
    for (const row of sections[key].rows) {
      ...
      const cat = ruleCat[row.id] || RULE_ID_TO_CAT[row.id];
...
export async function restoreDeleted(recordId) {
  ...
  for (const key of ['green', 'yellow', 'red']) {
    editedSections[key] = {
      rows: fileModel.sections[key].rows.map((r) => ({
        ...
        category: ruleCat[r.id] || RULE_ID_TO_CAT[r.id] || undefined,
  ...
  // recordId is the rule's original ID, so the static map can recover the
  // category even for history records written before categories were tracked.
  const restoredCat = rec.category || RULE_ID_TO_CAT[rec.recordId] || undefined;
  editedSections[target].rows.push({ id: null, cells, access: rec.access, category: restoredCat, icon: rec.icon || undefined });
