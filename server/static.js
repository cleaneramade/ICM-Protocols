...
export async function serveStatic(req, res, url) {
  // A malformed %-sequence (e.g. GET /%ZZ) throws URIError here. Unguarded, it
  // becomes an unhandled rejection and the socket hangs with no response — so
  // catch it and 404, exactly as the /api router already does for its params.
  let rel;
